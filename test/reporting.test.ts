...
registerHooks({ resolve(specifier, context, next) {
  ...
  if ((specifier === "@earendil-works/gondolin" && !context.parentURL?.includes("node_modules")) ||
      (specifier === "node:child_process" && context.parentURL?.endsWith("/src/guest-image.ts"))) {
    return { url: new URL("./fixtures/extension-boundaries.ts", import.meta.url).href, shortCircuit: true };
  }
...
async function setup(): Promise<TestHarness> {
  ...
  extension({ on: (name, fn) => handlers.set(name, fn), registerTool: () => {},
    registerCommand: (name, command) => commands.set(name, command) } as any);
  const ctx = { hasUI: true, ui: { confirm: async () => true,
    notify: (s: string) => notifications.push(s), setStatus: () => {},
...
function spawnInstance(h: TestHarness, confirm?: (action: unknown) => Promise<boolean>): {
  handlers: Map<string, Function>;
  commands: Map<string, any>;
  ctx: any;
} {
  ...
  const ctx = { hasUI: true, ui: {
    ...
    notify: (s: string) => h.notifications.push(s), setStatus: () => {},
    theme: { fg: (_color: string, s: string) => s } } };
...
function writeBuildProject(dir: string): void {
  ...
  fs.writeFileSync(path.join(dir, ".echoriad.json"),
    JSON.stringify({ buildConfig: "build.json" }));
  fs.writeFileSync(path.join(dir, "build.json"), JSON.stringify({
    arch: "aarch64", distro: "alpine", alpine: { version: "3.23.0" },
...
test("/echoriad keeps the existing report for a direct image selector", async () => {
  const h = await setup();
  try {
    fs.writeFileSync(path.join(h.dir, ".echoriad.json"),
