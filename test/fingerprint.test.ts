...
test("fingerprint covers recognized local inputs", () => {
  ...
  assert.deepEqual(result.localInputPaths, [
    p("init.sh"),
    p("helper"),
  ]);
});
...
test("file mode changes the fingerprint when it is build-relevant", () => {
  ...
  const exec = computeBuildFingerprint(
...
test("input node shapes cover file, symlink, and directory", () => {
  ...
  const file = hashLocalInput(dir, "f").node as Extract<
    LocalInputNode,
    { type: "file" }
  >;
  ...
  const link = hashLocalInput(dir, "l").node as Extract<
    LocalInputNode,
    { type: "symlink" }
  >;
  assert.equal(link.target, "f");
  const directory = hashLocalInput(dir, "d").node as Extract<
    LocalInputNode,
    { type: "directory" }
  >;
