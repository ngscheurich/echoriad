...
test("the authorization file lives under the Echoriad cache directory", () => {
  ...
  try {
    ...
    assert.equal(
      authorizationFilePath(),
      path.join(xdg, "echoriad", "image-authorizations.json"),
    );
  } finally {
...
test("malformed metadata is moved aside with a warning and treated as empty", () => {
  ...
  const associations = readAuthorizations(file, (message) =>
...
test("structurally invalid metadata is moved aside too", () => {
  ...
  fs.writeFileSync(
    file,
    JSON.stringify({ version: 1, associations: [{ consumer: 42 }] }),
  );
  const warnings: string[] = [];
  assert.deepEqual(
...
test("an interrupted write leaves the destination intact and removes only its own temporary file", () => {
  ...
  assert.deepEqual(
    remaining.filter(
      (name) =>
        name.startsWith("image-authorizations.json.tmp-") &&
        name !== path.basename(strayTemp),
    ),
    [],
  );
