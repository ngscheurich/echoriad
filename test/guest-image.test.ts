...
function resolveCliPath(): string {
  const pkg = path.dirname(
    fs.realpathSync(
      new URL("../node_modules/@earendil-works/gondolin/package.json", import.meta.url).pathname,
    ),
  );
...
test("build-failure errors carry a bounded tail of Gondolin output", () => {
  const output = Array.from({ length: 30 }, (_, i) => `step ${i + 1}`).join("\n");
...
test("privileged container warning appears only for container builds with postBuild commands", () => {
  ...
  // run privileged.
  const expected =
    process.platform !== "linux" || (hostArch !== undefined && hostArch !== withCommands.arch);
...
test("build failure stops startup, notifies the output tail, and removes the temporary output directory", async (t) => {
  ...
  assert.equal(error.message, "Echoriad: guest image build failed (Gondolin exited with code 1)");
...
process.exit(3);`,
  ...
  try {
    await runGondolinBuild({ ...command, cliPath: script }, undefined, onBuildFailure);
  } finally {
...
test("build configs rejected by Gondolin fail with an actionable error", async (t) => {
  ...
  await assert.rejects(
    ...
    (error: GuestImageError) => /rejected by Gondolin/.test(error.message) && error.permanent,
  );
});
