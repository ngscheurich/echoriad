...
test("an already cancelled build never starts a child", async () => {
	...
	await assert.rejects(
		runGondolinBuild(command, (s) => output.push(s), undefined, AbortSignal.abort()),
		/cancelled/,
	);
...
test("builds inherit host variables without guest environment filtering", async () => {
	...
	try {
		let output = "";
		await runGondolinBuild({ ...command, configPath: "environment" }, (s) => {
			output += s;
		});
