...
test("Pi routes build failures only to humans and tools fail without host details", async (t) => {
  ...
  fs.writeFileSync(path.join(dir, "build.json"), JSON.stringify({ arch: "aarch64", distro: "alpine", alpine: { version: "3.23.0" } }));
  process.chdir(dir);
  ...
  const ctx = { hasUI: true, ui: { confirm: async () => true,
    notify: (s: string) => human.push(s), setStatus: (_key: string, s: string) => human.push(s),
