...
function describeNode(
  ...
  if (node.isDirectory()) {
    ...
    for (const name of names) {
      ...
      entries[name] = describeNode(declaredPath, configDir, childPath, fs.lstatSync(childPath));
    }
...
export function hashLocalInput(
  ...
  const key = path.relative(configDir, absolutePath).split(path.sep).join(path.posix.sep);
  return {
    key,
 ...
 */
export function computeBuildFingerprint(config: BuildConfig, configDir: string): BuildFingerprint {
